...
WIDTH = 900
HEIGHT = 500
WIN = pygame.display.set_mode((WIDTH, HEIGHT)) #Vindue 
...
RED_HIT = pygame.USEREVENT + 2 #En slags event id 
...
def handle_bullets(yellow_bullets, red_bullets, yellow, red):
    for bullet in yellow_bullets:
        bullet.x += BULLET_VEL
        if red.colliderect(bullet): #only if both are rectangles
    ...
    for bullet in red_bullets:
        bullet.x -= BULLET_VEL
        if yellow.colliderect(bullet): #only if both are rectangles
...
def main(): #Main game loop 
    ...
    while run: 
        clock.tick(FPS)
        for event in pygame.event.get(): #Checker for forskellige events 
